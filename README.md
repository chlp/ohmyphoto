...
- **Soft enforcement**: only *suspicious* album requests count towards the per-IP counter: a wrong secret (403) or an unknown album (404) sent without a Turnstile token and without a valid bypass cookie. Opening a valid link never moves the counter, so the photographer testing links or many visitors behind one NAT never see the captcha. Turnstile is only *required* for an IP after it accumulates more than `TURNSTILE_SOFT_THRESHOLD` such requests within `TURNSTILE_SOFT_WINDOW_MS`. Before the threshold is exceeded, requests proceed without waiting; if a token is provided, the Worker verifies it in the background and only increments the counter if verification fails. A response with a valid secret always sets the bypass cookie, so returning browsers skip the check entirely.
- **Soft enforcement env vars**:
  - `TURNSTILE_SOFT_THRESHOLD` (default: `256`)
