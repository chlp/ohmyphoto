...
| `GET albums?cursor=` | One page (`PAGE_ALBUMS`) of `{ albumId, title, secretCount, secrets, fileCount }` plus `{ cursor, done }` (delimiter listing + one `info.json` read per album; `fileCount` counts only valid entries, same as the gallery). Client follows `cursor` |
