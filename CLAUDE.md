...
- Testing the writer outside a browser: the CRC/ZIP code is plain JS, so it can be extracted and run in Node, then checked with `unzip -t` and `ditto -x -k`. For a browser smoke test set `window.showSaveFilePicker = undefined` before clicking to force the Blob path (the native save dialog cannot be driven by automation).
