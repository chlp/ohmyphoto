...
albums/<albumId>/info.json       # { title, secrets, files: [ { name, ref }, ... ] }
```

- An album is **only its `info.json`**. `files` is the authoritative photo list: the public album endpoint never lists R2; if `files` is missing it returns 500. Each entry is `{ name, ref }`; `name` is the display name, unique within an album; `ref` is the storage identity and may appear in many albums. Anything else in `files` is ignored. Parsing lives in `src/utils/albumFiles.js` (`parseFileEntries`, `imageObjectKey`). The gallery renders entries in array order; admin writes keep `files` sorted with `localeCompare` and de-duplicated by name.
...
| `GET album/<id>/files` | Entries `{ name, ref, photoUrl, previewUrl }` with admin raw URLs |
...
| `POST album/<id>/file` | `multipart/form-data`: `photo`, `preview` (both must start with JPEG magic `FF D8 FF`), optional `name`, `ref` (sha256 hex of the source), `overwrite=1`. Shared objects are written only if missing (or overwrite); 409 if `name` exists in the album with a different ref. Returns `{ name, ref, stored }`. With `ref` supplied the photo is streamed, otherwise buffered once to hash it |
...
| `POST album/<id>/verify-files` | `head()` every entry's objects; drop entries whose photo is missing; `{ fileCount, removed, missingPreviewCount }`. No listing |
