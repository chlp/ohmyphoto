...
- **Photo deep links**: `ui.js:parseAlbumHash` splits the fragment into `{ secret, photoRef }`; the inline early-fetch script does the same split (`split('/')[0]`), keep both in sync. Opening the viewer pushes **one** history entry with state `{ ohmyphotoLightbox: 1 }` (`pushPhotoHash`) so the browser's Back button closes it; `showLightboxIndex` then only rewrites that entry to `#<secret>/<ref>` via `history.replaceState`. `closeLightbox` (Esc, ✕, backdrop) calls `history.back()` when that state is present and lets the `popstate` handler hide the viewer (`hideLightbox`, which restores `#<secret>` in place); Forward re-opens it on the ref in the hash (`openLightboxByIndex(idx, { pushHistory: false })`). A deep link first rewrites its own entry to `#<secret>` and then pushes the photo entry, so the first Back shows the album instead of leaving the site. The logo link always points at the album without a ref. After the grid renders, a `photoRef` opens the matching item (`findLightboxIndexByRef`: exact `ref`, else a unique prefix of ≥8 chars); an unknown ref strips itself from the URL and shows a status note. The album API exposes `ref` on every file for this. The lightbox bar has a `Copy link` button (`copyPhotoLink`: `navigator.clipboard.writeText` with a hidden-textarea `execCommand('copy')` fallback, label flips to `Copied` / `Copy failed` for 1.8 s).
